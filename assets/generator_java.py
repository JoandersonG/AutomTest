...
def generate_param_value_java(MUT, i, j):  # i = parameter order / j = testset order
...
def generate_expected_output_java(MUT, i):  # i = testset order
...
def test_content_java(MUT, test_set_name, cont, testset_position):
    ...
    for x in range(0, len(MUT.params)):
        if (x == 0):
            content += generate_param_value_java(MUT, x, testset_position)
        else:
            content += ", " + generate_param_value_java(MUT, x, testset_position)

    content += ");\n\t\tassertTrue("
    content += generate_expected_output_java(MUT, testset_position)
    content += ");\n\t}\n"
    return content


def generate_tests_java(MUT, file_path=''):
    ...
    if len(previous) == 0:
        testfile.write(header_content_java(MUT))
    else:
        testfile.seek(0)
        testfile.truncate()
    ...
    for i in range(0, len(MUT.testsets)):
        for j in range(0, MUT.testsets[i].number_of_cases):
            testfile.write(test_content_java(MUT, MUT.testsets[i].name, cont, i))
            cont += 1
